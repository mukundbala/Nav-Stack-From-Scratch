...
Astar::Astar():GridPlannerCore()
{
    ROS_INFO("[GridPlannerCore - Astar]: Astar ready for prep");
}

Astar::Astar(std::string cost_mode, bot_utils::MapData &map_data):
GridPlannerCore(cost_mode,map_data)
{
    ROS_INFO("[GridPlannerCore - Astar]: Astar Ready");
}

Astar::~Astar()
{
    ROS_INFO("[Astar]: Destroying Astar");
}

std::vector<bot_utils::Pos2D> Astar::plan(bot_utils::Pos2D idx_start, bot_utils::Pos2D idx_goal , bot_utils::MapData &map_data)
{
...
std::vector<bot_utils::Pos2D> Astar::plan(bot_utils::Index idx_start, bot_utils::Index idx_goal , bot_utils::MapData &map_data)
{   
    //path in indices
    ...
    while (open_list_.getSizeQ() != 0)
    {
        ...
        {   
            ...
            ROS_INFO("[Astar]: Found Goal!");
            ROS_INFO("[Astar]: Backtracking to Start...");
...
std::vector<bot_utils::Pos2D> Astar::post_process_path(std::vector<bot_utils::Pos2D>& raw_path , bot_utils::MapData &map_data)
{
    if (raw_path.size() <= 2)
    {   
        ROS_INFO("[Astar]: Path only contains 2 points or less. Returning Path!");
        return raw_path;  
    }
    ROS_INFO("[Astar]: Sparsifying");
    std::vector<bot_utils::Pos2D> sparse_path = sparsifyPath(raw_path);

    ROS_INFO("[Astar]: Making Any Angle!");
    std::vector<bot_utils::Pos2D> any_angle_path = makeAnyAnglePath(sparse_path , map_data);

    ROS_INFO("[Astar]: Returning Final Path");
